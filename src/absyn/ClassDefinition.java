...
public class ClassDefinition extends Absyn {
    ...
    public Program translate() {
    	...
    	ClassType selectedClass=staticType;
    	
    	if(selectedClass.isStartClass()){
	    	for(TestSignature t:selectedClass.getTests().values())
	    		t.getAbstractSyntax().translate(done);
	    	
    	for(FixtureSignature c:selectedClass.getFixtures())
    		c.getAbstractSyntax().translate(done);
    	}
    	
    	while(!selectedClass.getName().equals("Object")){
    		
    		
        	
        	for(ConstructorSignature c:selectedClass.getConstructors())
        		c.getAbstractSyntax().translate(done);
